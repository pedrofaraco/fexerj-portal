...
def validate(name, data, bio, games):
    ...
    if BIO_MARKER not in data:
        raise ValueError(f"{name}: bio marker (a5ff8944) not found — unsupported format")
    if PAIRING_MARKER not in data:
        raise ValueError(f"{name}: pairing marker (b3ff8944) not found — unsupported format")
    if not bio:
        raise ValueError(f"{name}: no players parsed from bio section — unsupported format")
...
def parse_tunx_from_bytes(data: bytes, name: str = "<binary>"):
    """Parse a Swiss Manager binary file from raw bytes.
...
def parse_tunx(filepath):
    ...
    with open(filepath, 'rb') as f:
        data = f.read()
    return parse_tunx_from_bytes(data, name=filepath)
