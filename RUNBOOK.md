...
The script SSHs into the NAS, fast-forwards the checked-out branch, rebuilds the frontend, and runs `docker compose ... up -d --build`. On any failure it automatically resets the NAS working copy to the previous commit and rebuilds — the previous version is restored without manual intervention.
