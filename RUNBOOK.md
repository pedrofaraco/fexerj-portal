...
The script SSHs into the NAS, fast-forwards the checked-out branch, rebuilds the frontend, and runs `docker compose ... up -d --build`. Before building the **`nginx`** image it sets **`BUILD_COMMIT`** (`git rev-parse --short HEAD`) and **`BUILD_FRONTEND_SNAPSHOT`** (`git rev-parse --short HEAD:frontend`, the short hash of the **`frontend/`** tree). The footer shows **`BUILD_FRONTEND_SNAPSHOT`** prominently: **matching values on UAT and prod mean the same `frontend/` snapshot**, even when **`BUILD_COMMIT` differs** (e.g. `develop` vs `master` branch tips). Manual **`docker compose ... --build`** should run both exports from the deploy directory: `export BUILD_COMMIT=$(git rev-parse --short HEAD)` and `export BUILD_FRONTEND_SNAPSHOT=$(git rev-parse --short HEAD:frontend)`. On any deploy failure it automatically resets the NAS working copy to the previous commit and rebuilds — the previous version is restored without manual intervention.
